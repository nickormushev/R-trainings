#Some changes are made to the markdown variant
library("ggplot2")
library("tidyverse")
colnames(diamonds)

#print(diamonds, n = nrow(diamonds))
#a  <- 1:5
#tibble(a, a * 2)
#print(diamonds[diamonds$price > 15000,], n = nrow(diamonds[diamonds$price > 15000,]))
...
# б)
nrow(diamonds[diamonds$carat < 2 & diamonds$price > 15000,])/nrow(diamonds[diamonds$price > 15000,])
...
#Долният barplot показва условната вероятност, ако знаем каква е 
...
#диаманта да е с по-хубава чистота. Тоест ако шлифовката е идеално е доста
...
#На долния barplot се вижда нещо подобно на картинката на по-миналия, само че
#разпределено между типовете. Вижда се, че каквато и шлифовка да имаме е по-вероятно
#тя да не е от свръх чист или много мръсен диамант, а нещо по-средата, като SI1/VS2.
#Там обаче и вероятността за лоша шлифовка е по-голяма. При по-чистите диаманти от друга страна
#вероятността за по-добра шлифовка
#barplot(prop.table(table(diamonds$cut, diamonds$clarity), 1), legend = T, 
#        beside = T, col = c( "black", "green", "yellow", "purple", "pink")) 
...
abline(l)
#от графиката се вижда, че са зависи, както и може да се 
#начертае проста линейна регресия, която да описва връзката
#между данните.
